-- フォームシナリオ移行の配信問題修正（個別・一括登録と同じロジック）
-- waitingステータスのままでscheduled-step-deliveryに処理を委ね、readyへの更新をしない
...
BEGIN
  ...
  IF v_scenario_id IS NOT NULL AND NEW.friend_id IS NOT NULL THEN
    ...
    -- 3. 新シナリオのトラッキングを作成（waitingステータスのみ、個別・一括登録と同じ）
    INSERT INTO public.step_delivery_tracking (
      scenario_id, step_id, friend_id, status, created_at, updated_at
    )
    SELECT
      v_scenario_id,
      s.id,
    ...
    -- scheduled-step-delivery関数をキック（個別・一括登録と同じ方法）
    BEGIN
