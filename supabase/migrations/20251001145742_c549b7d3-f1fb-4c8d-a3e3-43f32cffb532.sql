-- Add prevent_re_registration column to step_scenarios table
ALTER TABLE step_scenarios 
ADD COLUMN prevent_re_registration boolean NOT NULL DEFAULT true;

COMMENT ON COLUMN step_scenarios.prevent_re_registration IS 'シナリオレベルでの再登録制御: true = 再登録禁止, false = 再登録許可';

-- Update register_friend_to_scenario function to support hierarchical re-registration control
...
BEGIN
  ...
  ELSE
    -- 招待コード設定がない場合、シナリオ設定の逆を使用
    -- prevent_re_registration = true → allow_re_registration = false
