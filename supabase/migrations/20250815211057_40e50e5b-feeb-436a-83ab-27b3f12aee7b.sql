-- フォーム回答後のシナリオ遷移処理の修正（JSON エラー対応）
-- 文字列連結 + ::jsonb キャストを jsonb_build_object に変更し、HTTP 失敗時のロールバック防止を追加
...
BEGIN
  ...
  --   p_action  := 'form_insert_debug',
  --   p_details := json_build_object('data', NEW.data, 'meta', NEW.meta, 'form_id', NEW.form_id),
  --   p_success := true
  -- );
  ...
  IF v_scenario_id IS NOT NULL AND NEW.friend_id IS NOT NULL THEN
    ...
    EXCEPTION WHEN OTHERS THEN
      PERFORM public.log_security_event(
        ...
        p_details := json_build_object('error', SQLERRM, 'form_id', NEW.form_id, 'friend_id', NEW.friend_id),
        p_success := false
      );
    END;
  END IF;
...
-- トリガーを AFTER INSERT で張り直す
DROP TRIGGER IF EXISTS handle_form_scenario_transition_trigger ON public.form_submissions;

CREATE TRIGGER handle_form_scenario_transition_trigger
