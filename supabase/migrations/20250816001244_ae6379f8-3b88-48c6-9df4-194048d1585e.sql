-- 開発者アカウント管理のためのテーブルとプラン管理機能を追加（修正版）
...
-- 開発者用の売上集計関数（RLS対応）
CREATE OR REPLACE FUNCTION public.get_user_revenue_stats()
RETURNS TABLE(
  user_id UUID,
  display_name TEXT,
  plan_type plan_type,
...
AS $$
  ...
  WHERE up.is_active = true
    AND EXISTS (
      SELECT 1 FROM public.profiles dev_p
      WHERE dev_p.user_id = auth.uid() 
      AND dev_p.user_role = 'developer'
    );
$$;