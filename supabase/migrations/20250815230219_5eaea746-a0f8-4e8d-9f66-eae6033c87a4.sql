-- フォーム回答後のステップ配信問題調査と修正

-- 1. step_scenariosテーブルに「解除防止」フラグを追加
ALTER TABLE public.step_scenarios 
ADD COLUMN IF NOT EXISTS prevent_auto_exit boolean NOT NULL DEFAULT false;

-- 2. handle_form_scenario_transition関数を修正
-- 解除防止設定を考慮し、scheduled_delivery_atを適切に設定
...
BEGIN
  ...
  IF v_scenario_id IS NOT NULL AND NEW.friend_id IS NOT NULL THEN
    ...
    -- prevent_auto_exit = falseのシナリオのみ解除する
