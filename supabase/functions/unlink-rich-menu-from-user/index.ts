...
serve(async (req) => {
  ...
  try {
    ...
    const unlinkUrl = `https://api.line.me/v2/bot/user/${userId}/richmenu`;
    const unlinkResponse = await fetch(unlinkUrl, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${accessToken}` },
    });

    if (!unlinkResponse.ok) {
      throw new Error(`LINE API error (unlinking menu): ${await unlinkResponse.text()}`);
    }
