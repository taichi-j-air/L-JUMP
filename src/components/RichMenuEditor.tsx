...
export const RichMenuEditor = ({ menu, onSave, onCancel }: RichMenuEditorProps) => {
  ...
  const loadAvailableRichMenus = async () => {
    ...
    const { data, error } = await supabase.from('rich_menus').select('id, name, line_rich_menu_alias_id').eq('user_id', userData.user.id).order('name');
    ...
    try {
      const payload = {
        ...
        tapAreas: tapAreas.map(a => ({
            ...a,
            // For richmenuswitch, action_value should be the alias ID, not the DB ID
            action_value: a.action_type === 'richmenuswitch' 
                ? (availableRichMenus.find(rm => rm.id === a.action_value)?.line_rich_menu_alias_id || a.action_value) 
                : a.action_value
        })),
        isDefault,
      };
