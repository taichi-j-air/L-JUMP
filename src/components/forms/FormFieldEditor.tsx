...
export default function FormResponses() {
  ...
  useEffect(() => {
    const loadForms = async () => {
      ...
      try {
        ...
        // デフォルトで全てのフォームの通知を有効にする
        const updatedEnabledMap = { ...enabledMap };
        items.forEach(item => {
          if (updatedEnabledMap[item.id] === undefined) {
            updatedEnabledMap[item.id] = true; // デフォルトで有効
          }
        });
        
        setBadgeEnabledMap(updatedEnabledMap);
        localStorage.setItem('formBadgeEnabled', JSON.stringify(updatedEnabledMap));
  ...
  useEffect(() => {
    const loadSubmissions = async () => {
      ...
      // フォーム選択時に該当フォームの未読をクリア
      try {
        const raw = localStorage.getItem('unreadResponses');
        const counts = raw ? JSON.parse(raw) : {};
        if (counts[selectedForm] > 0) {
          counts[selectedForm] = 0;
          localStorage.setItem('unreadResponses', JSON.stringify(counts));
          setUnreadCounts(counts);
          window.dispatchEvent(new Event('unread-responses-updated'));
        }
      } catch (e) {
        console.error('未読クリアエラー:', e);
      }
    };
    loadSubmissions();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        (payload: any) => {
          ...
          if (!row) {
            console.log('No row data in payload');
            return;
          }

          console.log('Processing new submission:', row);

          // Append to list if this form is open
          if (row?.form_id === selectedForm) {
            console.log('Adding submission to current form view');
            setSubmissions(prev => [
              { 
                id: row.id, 
          ...
          try {
            ...
            console.log('Badge enabled map:', enabledMap);
            console.log('Form ID:', row.form_id, 'Enabled:', enabledMap[row.form_id]);
            
            // デフォルトで有効（明示的にfalseでない限り）
            if (enabledMap[row.form_id] !== false) {
              const raw = localStorage.getItem('unreadResponses');
              const map: Record<string, number> = raw ? JSON.parse(raw) : {};
              map[row.form_id] = (map[row.form_id] || 0) + 1;
              
              console.log('Updated unread counts:', map);
              ...
              console.log('Badge disabled for form:', row.form_id);
            }
          } catch (e) {
            console.error('未読カウント更新エラー:', e);
          }
        }
      )
      .subscribe((status) => {
    ...
    const handleUnreadUpdate = () => {
      try {
        const raw = localStorage.getItem('unreadResponses');
        const counts = raw ? JSON.parse(raw) : {};
        console.log('Updating unread counts from storage:', counts);
        setUnreadCounts(counts);
      } catch (e) {
        console.error('未読更新エラー:', e);
      }
    };
          ...
          <FormListTable
            ...
            onToggleBadge={(id) => {
              setBadgeEnabledMap((prev) => {
                ...
                console.log('Badge toggled for form:', id, 'New state:', !enabled);
                window.dispatchEvent(new Event('unread-responses-updated'));
                return next;
              });
            }}
