...
export default function PaymentManagement() {
  ...
  const loadOrders = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('orders').select('*').eq('user_id', user.id).eq('livemode', !showTestMode).order('created_at', {
        ascending: false
      });
      ...
      const monthlyRevenue = successful.filter(order => {
      ...
      // ユニークなサブスク加入者数
      const uniqueSubscriptionUsers = new Set(subscriptionOrders.filter(order => order.status === 'paid').map(order => order.friend_uid).filter(Boolean)).size;
      ...
      // ユニークな単発決済ユーザー数
      const uniqueOneTimeUsers = new Set(oneTimeOrders.filter(order => order.status === 'paid').map(order => order.friend_uid).filter(Boolean)).size;
  ...
  const loadFriends = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('line_friends').select('id, display_name, short_uid, line_user_id').eq('user_id', user.id).order('display_name');
      if (error) throw error;
      setFriends(data || []);
    } catch (error) {
  ...
  const loadProducts = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('products').select('id, name, price, product_type, currency').eq('user_id', user.id).eq('is_active', true).order('name');
      if (error) throw error;
      setProducts(data || []);
    } catch (error) {
  ...
  const handleCancelSubscription = async (customerId: string) => {
    try {
      ...
      if (data?.success) {
        ...
        setOrders(prevOrders => prevOrders.map(order => order.stripe_customer_id === customerId && order.metadata?.product_type?.includes('subscription') ? {
          ...order,
          status: 'canceled'
        } : order));
  ...
  const filteredOrders = orders.filter(order => {
    const matchesSearch = !searchTerm || order.stripe_session_id?.toLowerCase().includes(searchTerm.toLowerCase()) || order.friend_uid?.toLowerCase().includes(searchTerm.toLowerCase());
  ...
  const getProductName = (order: OrderRecord) => {
    ...
    if (order.product_id) {
      ...
      return <span className="bg-destructive text-white px-1 py-0.5 rounded text-xs font-medium">
          [削除された商品]
        </span>;
    }
    return 'アンノーン';
      ...
      <div className="container mx-auto px-4">
        <div className="mb-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <span className={`text-sm font-medium px-2 py-1 rounded ${showTestMode ? 'bg-amber-100 text-amber-800 border border-amber-300' : 'bg-emerald-100 text-emerald-800 border border-emerald-300'}`}>
              {showTestMode ? 'テストモード' : '本番環境'}
            </span>
            <Button variant="outline" size="sm" onClick={() => setShowTestMode(!showTestMode)} className={`flex items-center gap-2 ${showTestMode ? 'border-amber-300 hover:bg-amber-50' : 'border-emerald-300 hover:bg-emerald-50'}`}>
              ...
              <div className="flex gap-2">
                <Dialog open={showAddDialog} onOpenChange={setShowAddDialog}>
                  <DialogTrigger asChild>
                   ...
                   <DialogContent>
                     <Button variant="ghost" size="icon" className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground" onClick={() => setShowAddDialog(false)}>
                       <X className="h-4 w-4" />
                     </Button>
                     <DialogHeader>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="flex gap-2">
                          <Input id="friend-line-id" value={friendUid} onChange={e => setFriendUid(e.target.value)} placeholder="LINE IDを入力" />
                      ...
                      {selectedFriend && <div className="p-3 bg-muted rounded-md">
                      ...
                      <div>
                        ...
                        <Input id="product-name" value={manualProductName} onChange={e => setManualProductName(e.target.value)} placeholder="商品名を入力" />
                      </div>

                      <div>
                        <Label htmlFor="amount">金額（円）</Label>
                        <Input id="amount" type="number" value={manualAmount} onChange={e => setManualAmount(e.target.value)} placeholder="金額を入力" />
                      </div>

                      <div>
                        <Label htmlFor="manual-date">日付</Label>
                        <Input id="manual-date" type="datetime-local" value={manualDate} onChange={e => setManualDate(e.target.value)} />
                      </div>

                      <Button onClick={handleAddOrderManually} className="w-full" disabled={!selectedFriend || !manualAmount || !manualProductName}>
                        追加
                      </Button>
                    </div>
                  </DialogContent>
            ...
            <div className="space-y-3 mt-3">
              <div className="flex gap-2">
                <div className="flex-1">
                  <div className="relative">
                    ...
                    <Input placeholder="セッションIDまたは友達UIDで検索" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="pl-7 h-8 text-xs" />
                  </div>
                </div>
              ...
              <div className="flex flex-wrap items-center gap-4">
                <div className="flex items-center space-x-2">
                  <Checkbox id="show-pending" checked={showPendingOrders} onCheckedChange={checked => setShowPendingOrders(checked === true)} />
                  <Label htmlFor="show-pending" className="text-xs">
                    保留中の注文を表示
                  </Label>
                </div>
                <div className="flex items-center space-x-2">
                  <Checkbox id="show-canceled" checked={showCanceledOrders} onCheckedChange={checked => setShowCanceledOrders(checked === true)} />
                  <Label htmlFor="show-canceled" className="text-xs">
                    解約済みの注文を表示
                  </Label>
                </div>
                <div className="flex items-center space-x-2">
                  <Checkbox id="show-refunded" checked={showRefundedOrders} onCheckedChange={checked => setShowRefundedOrders(checked === true)} />
          ...
          <CardContent className="p-0">
            <div className="overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {filteredOrders.map(order => <TableRow key={order.id} className="text-xs">
                      ...
                      <TableCell className="py-2">
                        ...
                        {order.status === 'pending' && <div className="text-xs text-amber-600">保留中</div>}
                      </TableCell>
                       ...
                       <TableCell className="py-2">
                         <Badge variant={getStatusColor(order.status)} className="text-xs" style={getStatusStyle(order.status)}>
                           ...
                           {order.status === 'paid' && <AlertDialog>
                               ...
                               <AlertDialogContent>
                                 ...
                                 <AlertDialogFooter>
                                   ...
                                   <AlertDialogAction onClick={() => handleRefund(order.id)} className="bg-destructive text-destructive-foreground hover:bg-destructive/80">
                                     返金実行
                                   </AlertDialogAction>
                                 </AlertDialogFooter>
                               </AlertDialogContent>
                             </AlertDialog>}
                           {order.stripe_customer_id && order.status === 'paid' && order.metadata?.product_type?.includes('subscription') && <AlertDialog>
                               <AlertDialogTrigger asChild>
                               ...
                               <AlertDialogContent>
                                 ...
                                 <AlertDialogFooter>
                                   <AlertDialogCancel>キャンセル</AlertDialogCancel>
                                   <AlertDialogAction onClick={() => handleCancelSubscription(order.stripe_customer_id)} className="bg-destructive text-destructive-foreground hover:bg-destructive/80">
                                     解約実行
                                   </AlertDialogAction>
                                 </AlertDialogFooter>
                               </AlertDialogContent>
